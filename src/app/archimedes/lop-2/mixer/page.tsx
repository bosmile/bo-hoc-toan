...
const SampleClockComponent = () => (
  <div className="col-span-full flex flex-row items-center justify-center gap-6 p-4 border-2 border-dashed border-primary/30 rounded-3xl bg-primary/5 mb-4 break-inside-avoid no-print:shadow-sm">
    <div className="bg-white p-2 rounded-full shadow-inner border border-slate-100 shrink-0">
      <AnalogClock 
        hour={10} 
        minute={10} 
        showHands={false} 
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1.5">
        <div className="px-3 py-1 bg-white rounded-lg border border-slate-200 shadow-xs flex items-center gap-2">
          <div className="size-1.5 rounded-full bg-primary" />
          <p className="text-[10px] font-bold text-slate-600">Mỗi số = <span className="text-primary font-black">+5 phút</span></p>
        </div>
        <div className="px-3 py-1 bg-white rounded-lg border border-slate-200 shadow-xs flex items-center gap-2">
          <div className="size-1.5 rounded-full bg-blue-600" />
          <p className="text-[10px] font-bold text-slate-600">Vòng ngoài = <span className="text-blue-600 font-black">Số phút</span></p>
        </div>
      </div>
    </div>
  </div>
...
export default function ArchimedesMixerPage() {
  ...
  React.useEffect(() => {
    ...
    const savedTheme = localStorage.getItem('mixer2_theme') || 'garden';
    setTheme(savedTheme);
    ...
    const savedVersions = localStorage.getItem('mixer2_cart_versions');
    if (savedVersions) setExamVersions(JSON.parse(savedVersions));

    loadSetting('mixer2_cd1', setCd1Settings, null);
    loadSetting('mixer2_cd2', setCd2Settings, null);
    loadSetting('mixer2_cd3', setCd3Settings, null);
    loadSetting('mixer2_cd4', setCd4Settings, null);
    loadSetting('mixer2_cd5', setCd5Settings, null);
    loadSetting('mixer2_cd6', setCd6Settings, null);
    loadSetting('mixer2_cd7', setCd7Settings, null);
    loadSetting('mixer2_cd8', setCd8Settings, null);
    loadSetting('mixer2_cd9', setCd9Settings, null);
    loadSetting('mixer2_cd10', setCd10Settings, null);
    loadSetting('mixer2_cd11', setCd11Settings, null);
    loadSetting('mixer2_preset_counts', setPresetCounts, null);
    loadSetting('mixer2_hidden_topics', setHiddenTopics, null);
  }, [])
  
  React.useEffect(() => {
    if (mounted) localStorage.setItem('mixer2_cart_versions', JSON.stringify(examVersions));
  ...
  const toggleTheme = () => {
    ...
    setTheme(next);
    localStorage.setItem('mixer2_theme', next);
  ...
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_cd1', JSON.stringify(cd1Settings)) }, [cd1Settings, mounted])

  const [cd2Settings, setCd2Settings] = React.useState({ count: 5, tables: [2, 5, 10], unknownVariable: "C" as any, shuffle: true })
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_cd2', JSON.stringify(cd2Settings)) }, [cd2Settings, mounted])

  const [cd3Settings, setCd3Settings] = React.useState({ count: 5, level: "1" as any, maxRange: 20, operationMode: "mixed" as any })
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_cd3', JSON.stringify(cd3Settings)) }, [cd3Settings, mounted])
  ...
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_cd4', JSON.stringify(cd4Settings)) }, [cd4Settings, mounted])

  const [cd5Settings, setCd5Settings] = React.useState({ count: 2, cycleLength: 3, maxCycleSum: 30 })
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_cd5', JSON.stringify(cd5Settings)) }, [cd5Settings, mounted])

  const [cd6Settings, setCd6Settings] = React.useState({ count: 2, size: "4" as any, difficulty: "easy" as any })
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_cd6', JSON.stringify(cd6Settings)) }, [cd6Settings, mounted])

  const [cd7Settings, setCd7Settings] = React.useState({ count: 3, difficulty: "hours" as any, type: "read" as any })
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_cd7', JSON.stringify(cd7Settings)) }, [cd7Settings, mounted])

  const [cd8Settings, setCd8Settings] = React.useState({ count: 2 })
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_cd8', JSON.stringify(cd8Settings)) }, [cd8Settings, mounted])

  const [cd9Settings, setCd9Settings] = React.useState({ count: 2, maxSum: 20, type: "chain" as any, allowSubtraction: false })
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_cd9', JSON.stringify(cd9Settings)) }, [cd9Settings, mounted])

  const [cd10Settings, setCd10Settings] = React.useState({ count: 1, maxSum: 100, difficulty: "easy" as any })
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_cd10', JSON.stringify(cd10Settings)) }, [cd10Settings, mounted])

  const [cd11Settings, setCd11Settings] = React.useState({ count: 2, topic: "Lego", difficulty: "Trung bình" as any })
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_cd11', JSON.stringify(cd11Settings)) }, [cd11Settings, mounted])

  const [hiddenTopics, setHiddenTopics] = React.useState<number[]>([])
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_hidden_topics', JSON.stringify(hiddenTopics)) }, [hiddenTopics, mounted])
  ...
  React.useEffect(() => { if (mounted) localStorage.setItem('mixer2_preset_counts', JSON.stringify(presetCounts)) }, [presetCounts, mounted])
